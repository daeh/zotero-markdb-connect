...
Zotero.MarkDBconnect = {
  ...
  init: async function () {
    ...
    const requirebbt = this.getPref('matchstrategy') === 'bbtcitekey'
        ...
        async function () {
          ...
          if (requirebbt) {
            ...
            win.Zotero.BetterBibTeX.ready.then(
              async function () {
                await this.startupDependencyCheck().bind(this)
              }.bind(this),
            )
          } else {
            await delayms(2000)
          ...
          Zotero.MarkDBconnect._initialized = true
        }.bind(this),
      )
    }
  },
    ...
    if (typeof metadatakeyword === 'string' && metadatakeyword.length > 0) {
      ...
      if (found.length > 0) {
        this.showNotification(
          'Invalid citekey metadata',
          'metadata id cannot contain: ' + found.join(' or ') + '.',
          false,
        )
        return false
      } else {
  ...
  _getParam_vaultpath: function () {
    try {
      ...
      this.showNotification(
        ...
        'Set the path to your notes in the MarkDBConnect preferences.',
        false,
      )
  ...
  _getParam_zotkeyregex: function () {
    try {
      ...
      this.showNotification(
        'User Defined RegEx Invalid',
        `The RegEx you specified in the MarkDBConnect preferences is invalid: ${e}`,
        false,
      )
      Zotero.debug(`MarkDBconnect: Error: ${e}`)
      return null
  ...
  chooseBBTjson: async function () {
    ...
    const filter = [['JSON File (*.json)', '*.json']]
    ...
    if (reserrs.length > 0) {
      ...
      if (promptSaveErrors) {
        ...
        const warningMessage =
          'There were ' +
          nerr.toString() +
          ' Markdown notes that could not be parsed. \n\nWould you like to save these errors to a json file? \n\n(There were ' +
          (res.length - reserrs.length).toString() +
        ...
        this.showNotification(
          'scanVault',
          'Unable to parse ' + nerr.toString() + ' of ' + filteredFiles.length.toString() + ' notes.',
          false,
        )
      }
    }
      ...
      filteredFiles.map(async (entry) => {
        ...
        try {
          ...
          if (typeof contents === 'string') {
            const re_matched = contents.match(re_contents)
            if (re_matched && re_matched.length > 1) entry_res.zotkeys.push(re_matched[1].trim())
          } else {
  ...
  _getBBTkeyData: async function () {
    try {
      if (Zotero.BetterBibTeX && typeof Zotero.BetterBibTeX === 'object' && Zotero.BetterBibTeX !== null) {
    ...
    items.forEach((item) => {
      ...
      if (itemIDs.includes(item.itemID)) {
        ...
        if (Object.keys(citekeymap).includes(item.citationKey)) {
          ...
          /// otherwise make array with zotero item id
    ...
    if (reserr.length > 0) {
      ...
      if (promptSaveErrors) {
        ...
        const warningMessage =
          'There were ' +
          nerr.toString() +
          ' citekeys in your Markdown notes that could not be matched to items in your Zotero library. \n\nWould you like to save the names of these citekeys in a json file? \n\n(Matches for ' +
          (res.length - reserr.length).toString() +
          ' citekeys were found successfully.)'
        const saveDialogTitle = 'Save MarkDBConnect Errors To...'
        const filenamesuggest = 'MarkDBConnect-missing-entries.json'
        await this.offerToSaveErrors(dataErrors, warningTitle, warningMessage, saveDialogTitle, filenamesuggest)
      } else {
        this.showNotification(
          'MarkDBConnect Warning',
          '' + nerr.toString() + ' unmatched citekeys. Run Sync from Tools menu to generate report.',
          false,
        )
      }
    }
    ...
    if (reserr.length > 0) {
      ...
      if (promptSaveErrors) {
        ...
        const warningMessage =
          'There were ' +
          nerr.toString() +
          ' zoteroKeys in your Markdown notes that could not be matched to items in your Zotero library. \n\nWould you like to save the names of these in a json file? \n\n(Matches for ' +
          (res.length - reserr.length).toString() +
          ' zoteroIDs were found successfully.)'
        const saveDialogTitle = 'Save MarkDBConnect Errors To...'
        const filenamesuggest = 'MarkDBConnect-missing-entries.json'
        await this.offerToSaveErrors(dataErrors, warningTitle, warningMessage, saveDialogTitle, filenamesuggest)
      } else {
        this.showNotification(
          'MarkDBConnect sliceObjCustomRegex Warning',
          '' + nerr.toString() + ' unmatched zoteroKeys. Run Sync from Tools menu to generate report.',
          false,
        )
      }
    }
  ...
  processData: async function (promptSaveErrors, debug) {
    ...
    if (matchstrategy === 'bbtcitekey') {
      ...
      if (res.length === 0) {
        if (this._getParam_filefilterstrategy() === 'default') {
          this.showNotification(
            ...
            'Check the path to your Markdown notes in the MarkDBConnect preferences.',
            false,
          )
        } else {
          this.showNotification(
            'No Markdown files found',
            'Check the File Filter RegEx and the path to your Markdown notes in the MarkDBConnect preferences.',
            false,
          )
        }
        return
      ...
      if (res.length === 0) {
        this.showNotification(
          'No Markdown files found',
          'Set the path to your Markdown notes in the MarkDBConnect preferences.',
          false,
        )
        return
      }
    ...
    if (this.dataKeys.length === 0) {
      this.showNotification(
        'No Matching Entries',
        'None of the ' + res.length.toString() + ' Markdown notes could be matched to items in the Zotero library.',
        false,
      )
    }
  ...
  updateItems: async function (citekeyids) {
    ...
    if (citekeyids.length != items_withnotes.length) {
      this.showNotification(
        'BBT JSON mismatch',
        '' + nitems_notlocatable.toString() + ' IDs in the BBT JSON could not be matched to items in the library',
        false,
      )
    }
        ...
        try {
          ...
          if (typeof old_obscite_version === 'string' && old_obscite_version.length > 0) {
            this.showNotification(
              ...[
                'MarkDBConnect',
                "ZotObsCite has been renamed. Please remove 'ZoteroObsidianCitations' from your Extensions",
                false,
              ],
            )
            return
  ...
  buildItemContextMenuListObsidian: function () {
    ...
    for (let node of nodes) node.setAttribute('hidden', true)
  ...
  openObsidianURI: function (entry_res) {
    ...
    const fileKey =
      uri_spec === 'file' ? 'file=' + encodeURIComponent(entry_res.name) : 'path=' + encodeURIComponent(entry_res.path)
