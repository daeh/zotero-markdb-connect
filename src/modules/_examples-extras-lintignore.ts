...
export class BasicExampleFactory {
  ...
  static registerNotifier() {
    const callback = {
      notify: async (event: string, type: string, ids: number[] | string[], extraData: Record<string, any>) => {
        if (!addon?.data.alive) {
...
export class UIExampleFactory {
  ...
  static registerRightClickMenuPopup() {
    ztoolkit.Menu.register(
      ...
      'before',
      document.querySelector('#zotero-itemmenu-addontemplate-test'),
    )
  }
          ...
          if (ids.length == 0) {
            ...
            text.split(/\s*(&&|\|\|)\s*/g).forEach((conditinString: string) => {
              ...
              if (conditions.length == 3 && operators.indexOf(conditions[1]) != -1) {
                ...
                s.addCondition(conditions[0], conditions[1] as Zotero.Search.Operator, conditions[2])
              }
            })
